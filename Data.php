...
class Data{
  ...
  public function get_data($data){
    ...
    $info .= "<div class='container'><div class='content'>";
    // loop to get data from the api
    foreach ($user_data as $key => $value) {
      if (in_array($value->attributes->title, $check)) {
        // if k is even show image first
        if ($k%2 == 0) {
          $info .= "<div class='main_image'>";
          $info .= "<div class='left'>".$this->get_image($value)."</div>";
          $info .= "</div>";
          $info .= "<div class='sec_content'>";
          $info .= "<div class='left'><h1>".$value->attributes->field_secondary_title->value."</h1></div>";
          $info .= "<div class='left'>".$this->get_icon($value)."</div>";
          ...
          $info .= "<div class='left'>".$temp."</div>";
          $info .= "<div class='button'><a href='http://innoraft.com".$value->attributes->path->alias."'>Explore More</a></div>";
          $info .= "</div>";
        }
        // show contet first
        else{
